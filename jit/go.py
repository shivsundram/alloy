...
class PointOp():
    ...
    def make_c_body(self, schedule,args, kernel_returns, node_dict):
        body = []
        body.append("{")
        body.append("\t#pragma omp parallel for")
...
# node: a node in the python ast
# graph_node: a node in the new ast data structure
...
# I/O, entry points for conversion  
...
def convertFunc2Native(foo):
    ...
    cmd = ["/usr/local/bin/gcc", "-O2", "-c", "-Wall", "-fopenmp", "-Werror", "-fpic", kernel_name+"GEN"+'.c']  
    p = subprocess.call(cmd);  
    print(name)
    cmd = ["/usr/local/bin/gcc", "-shared","-fopenmp", "-o", name+".so", name+".o"]
    p = subprocess.call(cmd);  
...
def timeCompare():
    ...
    d = np.ones((n), dtype='float32')
    e = np.ones((n), dtype='float32')

    d1 = np.ones((n), dtype='float32')
    e1 = np.ones((n), dtype='float32')
